...
Make a file called `.env` at the root of the project, and copy the contents of `.env.template` into it. Some tools in `api-tools` require certain environment variables, which you can fill in `.env`
If you're not sure what to put, please try contacting one of our members through discord.
